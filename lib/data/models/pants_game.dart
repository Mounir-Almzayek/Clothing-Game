...
class PantsGame extends FlameGame with PanDetector {
  ...
  Future<void> onLoad() async {
    ...
    pants = Pants(
      s0: initial[0], s1: initial[1],
      s2: initial[2], s3: initial[3],
      s4: initial[4], s5: initial[5],
      s6: initial[6], s7: initial[7],
    );
  ...
  void onPanEnd(DragEndInfo info) {
    if (_dragStartSegment != null && _dragLastSegment != null) {
      final moved = pants.determineMovement(
        _dragStartSegment!.row, _dragStartSegment!.col,
        _dragLastSegment!.row, _dragLastSegment!.col,
      );
      if (moved) {
        _updatePositions();
        onGridUpdate(pants.grid);
        _confetti.play();
      }
    }
  ...
  void firstNonEmptySegment() {
    if (pants.row0Left.isNotEmpty) {
      highlightSection(0,0,Colors.greenAccent,Duration(seconds: 1));
    }
    else if (pants.row3Left.isNotEmpty) {
      highlightSection(3,0,Colors.greenAccent,Duration(seconds: 1));
    }
    else if (pants.row0Right.isNotEmpty) {
      highlightSection(0,1,Colors.greenAccent,Duration(seconds: 1));
    }
    else if (pants.row3Right.isNotEmpty) {
      highlightSection(3,1,Colors.greenAccent,Duration(seconds: 1));
    }
  }
}